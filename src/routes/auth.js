...
export function setupAuthRoutes(app) {
  // Login documents and responses contain no reusable state and must never be cached.
  ...
  // Browser clients use the returned bearer token for API requests. Parsers are
  // route-local so login keeps working if application middleware order changes.
  app.post('/login', noStore, express.json({ limit: LOGIN_BODY_LIMIT }), express.urlencoded({ extended: false, limit: LOGIN_BODY_LIMIT }), async (req, res) => {
